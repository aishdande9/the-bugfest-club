...
const TripDetailById = () => {
  const { id } = useParams(); // Getting tripId from the URL
  ...
  useEffect(() => {
    const fetchTripPlan = async () => {
      setLoading(true);
      try {
        const response = await fetch(`http://localhost:8080/api/trips/tripPlan/${id}`);
        if (!response.ok) {
  ...
  return (
    <div className="max-w-4xl mx-auto overflow-hidden border bg-white shadow-md mt-10">
      <Link to="/explore" className="text-blue-600 hover:underline mb-4 inline-block ml-6">
        ← Back to Explore
      </Link>
        ...
        <div className="flex flex-col md:flex-row md:gap-8">
          ...
          <div className="w-full mb-6 md:w-3/5 md:mb-0">
            <h2 className="text-2xl font-semibold text-gray-800 mb-4">
              {tripPlan.tripTitle || "Trip Title"} in {tripPlan.location || "Trip Location"}
            </h2>
            <p className="text-gray-600 mb-4">{tripPlan.description || "Trip Description"}</p>
            ...
            <ul className="space-y-2">
              <li className="flex items-center text-gray-600 mb-3">
                ...
                <span>
                  <b>Highlight:</b> {tripPlan.topActivity || "Highlight Info"}
                </span>
              </li>
              <li className="flex items-center text-gray-600 mb-3">
                <span className="mr-2">⭐</span>
                <span>
                  <b>Experience:</b> {tripPlan.mainAttraction || "Activity Name"}
                </span>
              </li>
              <li className="flex items-center text-gray-600 mb-3">
                <span className="mr-2">⭐</span>
                <span>
                  <b>Vibe-Inspired Spot:</b> {tripPlan.vibeInspiration || "Vibe-related highlight"}
                </span>
              </li>
            </ul>
          </div>
            ...
            <div className="bg-gray-50 p-5 rounded-lg">
              ...
              <div className="mb-6 space-y-3">
                <div>
                  ...
                  <p className="text-sm text-gray-600 mb-1">{tripPlan.startingLocation}</p>
                </div>
